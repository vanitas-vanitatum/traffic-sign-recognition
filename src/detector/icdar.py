...
def get_images(path: Path) -> List[str]:
    ...
    files_list = list(map(lambda x: str(x.as_posix()), files_iter))
    return files_list
...
def line_cross_point(line1, line2):
    ...
    if line1[0] != 0 and line1[0] == line2[0]:
        tf.logging.warn('Cross point does not exist')
        return None
...
def sort_rectangle(poly):
    ...
    if np.count_nonzero(poly[:, 1] == poly[p_lowest, 1]) == 2:
        ...
        # assert angle > 0
        if angle <= 0:
            tf.logging.warn(angle, poly[p_lowest], poly[p_lowest_right])
...
def input_fn(data_path: Path, train: bool,
    ...
    y = {
        "score_maps": score_batch,
        "geo_maps": geo_batch,
        "training_masks": mask_batch
    }
    return x, y

...
def scaling_layer(inputs: tf.Tensor) -> tf.Tensor:
    ...
    with tf.name_scope("scale_shift_{}".format(scaling_layer_counter)) as name_scope:
        ...
        shifts = tf.get_variable(name_scope + "/shifts", (input_channels,), tf.float32, initializer=initializers.Zeros())
        scaling_layer_counter += 1
...
def elu_scale_concatenated_convo(inputs: tf.Tensor, features: int, kernel_size: int, stride: int):
...
def elu_scale_convo(inputs: tf.Tensor, features: int, kernel_size: int, stride: int):
    x = scaling_layer(inputs)
    x = tf.nn.elu(x)
...
def crelu_block(inputs: tf.Tensor, inner_features: int, outer_features: int, kernel_size: int,
    ...
    x = elu_scale_convo(inputs, inner_features, 1, 1)
    x = elu_scale_concatenated_convo(x, inner_features, kernel_size, stride)
    x = elu_scale_convo(x, outer_features, 1, 1)

    if input_features != outer_features or stride > 1:
        inputs = elu_scale_convo(inputs, outer_features, 1, stride)
    return x + inputs
...
def inception_block(inputs: tf.Tensor, features_1x1: int, features_3x3: Sequence[int],
    ...
    b1 = elu_scale_convo(inputs, features_1x1, 1, stride)

    b2 = elu_scale_convo(inputs, features_3x3[0], 1, stride)
    b2 = elu_scale_convo(b2, features_3x3[1], 3, 1)

    b3 = elu_scale_convo(inputs, features_5x5[0], 1, stride)
    b3 = elu_scale_convo(b3, features_5x5[1], 3, 1)
    b3 = elu_scale_convo(b3, features_5x5[2], 3, 1)
    ...
    if stride > 1:
        ...
        b4 = elu_scale_convo(b4, pool, 1, 1)
        to_concat.append(b4)

    result = tf.concat(to_concat, axis=-1)
    result = elu_scale_convo(result, out, 1, 1)
    return result
